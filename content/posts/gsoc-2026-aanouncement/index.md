...
Hello, I am Alok (aka [IndAlok](https://github.com/IndAlok)), an Electronics & Communication Engineering undergraduate at Indian Institute of Information Technology Jabalpur, India. I became interested in reverse engineering through CTFs and low-level security challenges, which is also how I started using Rizin and Cutter more often.
Rizin fits naturally into the way I approached CTF binaries from the CLI: inspecting functions, strings, references, and analysis output while scripting small workflows around them. Cutter complemented that by giving me a clearer visual view of control flow, xrefs, and types when I needed it.

One project that brought me much closer to the Rizin ecosystem was [RzWeb](https://github.com/IndAlok/rzweb), a browser-based Rizin/WebAssembly RE project. It started from a practical CTF need: I wanted a Rizin environment that could run from the browser and reduce setup friction when quickly inspecting binaries. 

Working on it made me look beyond user commands into browser-compatible I/O, frontend integration, and the boundary between Rizin as an analysis engine and the interface around it. That experience made me much more interested in exploring and contributing to the project.

### Project

Originally, I was selected for the [classes-analysis project](https://rizin.re/gsoc/2026/#classes-analysis-for-cobjectivecswiftdlangjava-350-hour-project), which focuses on improving object-oriented language analysis in Rizin. After discussing the exact scope with the mentors, it was decided to approach this from the dynamic-analysis side as well by integrating **Frida** into Rizin and Cutter, and using it to recover class information from running programs.

This summer, I will be working on **Frida-based dynamic class analysis** for Rizin and Cutter. The goal is to make Frida's runtime instrumentation available from inside the Rizin/Cutter workflow, so class information observed in a running target can be connected back to Rizin's analysis model instead of staying isolated in separate scripts and terminals.

The implementation will focus on a **Rizin backend first**, with support for connecting to Frida targets, attach/spawn/launch workflows, script execution, runtime information, memory access where reliable, events, and a practical debugging/tracing foundation. Cutter should expose the same backend through a small UI, without duplicating Frida logic in the GUI.

For **Android Java/Kotlin** targets, Frida's runtime APIs can expose information that is difficult to recover statically, such as loaded classes, class-loader context, methods, fields, inheritance, and related metadata. I aim to make that information available to Rizin in a structured way, so it can be reused by Rizin analysis and Cutter views as part of the broader class-analysis workflow.

I have also been exploring [r2frida](https://github.com/nowsecure/r2frida), which is a crucial listed reference because it shows how a RE shell can drive Frida through a native plugin and injected agent. For Rizin, the integration is expected to be native from the beginning: structured command output, explicit lifecycle handling, class-loader-aware recovery, Rizin class/type imports, Cutter integration, and tests around the core workflows.
